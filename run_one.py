...
if os.environ.get("REPLICATION_BACKEND", "libsumo") == "libsumo":
    if sys.platform == "win32":
        import sumo
        _sumo_dlls = os.add_dll_directory(os.path.join(os.path.dirname(sumo.__file__), "bin"))
...
def run_trial(controller_name, n, seed,
    ...
    random.seed(seed)  # The recovered runner omitted the controller RNG seed.
...
''')
    ...
    try:
        ...
        while traci.simulation.getMinExpectedNumber() > 0 and t < SIM_END:
            ...
            if 200 <= t <= 1400:
                occupancy.append(traci.vehicle.getIDCount())
        ctrl.teardown()
    finally:
        try: traci.close()
    ...
    completed = len(waits)
    # Empty-network early termination implies zero occupancy for the rest of
    # the fixed [200, 1400] measurement window, not a shorter denominator.
    mean_in_system = sum(occupancy) / 1201
    return {
        ...
        "departed": departed,
        "action_hold": getattr(ctrl, "action_counts", {}).get(0, 0),
        "action_expert": getattr(ctrl, "action_counts", {}).get(1, 0),
        "action_override": getattr(ctrl, "action_counts", {}).get(2, 0),
    }
